...
def get_optimal_move(game_state: Reversi, depth: int) -> Tuple[int, int]:
    ...
    if is_maximizing:
        ...
        for move in game_state.valid_moves_list:
            val = minimax(game_state, move, depth - 1, alpha, beta)
            if val > max_val:
                max_val = val
                optimal_move = move
                alpha = val
...
def minimax(game_state: Reversi, move: Tuple[int, int], depth: int, alpha: int, beta: int) -> int:
    ...
    if is_maximizing:
        ...
        for move in game.valid_moves_list:
            val = minimax(game, move, depth - 1, alpha, beta)
